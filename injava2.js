...
function printProfessionalCards() {
    ...
    let cardsHtml = '<div class="print-cards-container">';
            ...
            <div class="id-card-wrapper">
                <div class="pro-card front">
                    <div class="card-header-top">
                        <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/a/af/Algeria_map.svg/1200px-Algeria_map.svg.png" class="mini-map">
                        <div class="gov-text">
                            الجمهورية الجزائرية الديمقراطية الشعبية<br>
                            وزارة التربية الوطنية<br>
                            مديرية التربية لولاية توقرت
                        </div>
                        <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/d/d4/Flag_of_Algeria.svg/1200px-Flag_of_Algeria.svg.png" class="mini-flag">
                    </div>
                    
                    <h2 class="card-main-title">بطاقة مهنية</h2>
                    ...
                    <div class="card-body">
                        ...
                        <div class="user-info">
                            <div class="info-row"><span>اللقب:</span> <b>${emp.fmn}</b></div>
                            <div class="info-row"><span>الاسم:</span> <b>${emp.frn}</b></div>
                            <div class="info-row"><span>تاريخ الميلاد:</span> <b>${fmtDate(emp.diz)}</b></div>
                            <div class="info-row"><span>الوظيفة:</span> <b>${getJob(emp.gr)}</b></div>
                        </div>
                        <div class="card-side-meta">
                            رقم البطاقة: ${emp.ccp.slice(-4)}<br>
                            تاريخ الإصدار: ${new Date().toLocaleDateString('ar-DZ')}
                        </div>
                    </div>
                    
                    <div class="card-footer-nin">
                        رقم التعريف الوطني للموظف : ${emp.nin || '------------------'}
                    </div>
                    <div class="card-qr">
                         <img src="https://api.qrserver.com/v1/create-qr-code/?size=50x50&data=${emp.nin}">
                    </div>
                </div>

                <div class="pro-card back">
                    <img src="https://i.ibb.co/p6S9X70/logo-back.png" class="watermark-teacher">
                    <h3 class="back-title">وزارة التربية الوطنية</h3>
                    <ul class="back-rules">
                        <li>كل استعمال غير قانوني أو لأغراض غير مهنية للبطاقة المهنية يعرض صاحبها لعقوبات تأديبية.</li>
                        <li>في حالة التقاعد أو الإستقالة أو الخروج من الولاية يجب على الموظف تسليم البطاقة للإدارة.</li>
                    </ul>
                    <div class="back-seal">
                        مديرية التربية لولاية توقرت
                    </div>
    ...
    const printContainer = document.getElementById("printContainer");
    const originalContent = printContainer.innerHTML;
    printContainer.innerHTML = cardsHtml + getCardStyles(); // دمج الستايل مع المحتوى
...
function getCardStyles() {
    ...
    <style>
        ...
        .print-cards-container { display: flex; flex-wrap: wrap; gap: 20px; justify-content: center; direction: rtl; }
        
        .id-card-wrapper { display: flex; flex-direction: column; gap: 5px; page-break-inside: avoid; margin-bottom: 30px; }

        .pro-card {
            width: 85.6mm; height: 54mm; /* حجم بطاقة التعريف القياسي */
            border: 1px solid #ddd; border-radius: 10px; position: relative;
            background: #fff url('https://www.transparenttextures.com/patterns/cubes.png');
            box-shadow: 0 0 5px rgba(0,0,0,0.1); overflow: hidden; font-family: 'Cairo', sans-serif;
        }

        /* الوجه الأمامي */
        .card-header-top { display: flex; justify-content: space-between; padding: 5px 10px; background: rgba(255,255,255,0.8); }
        .mini-map, .mini-flag { height: 25px; }
        .gov-text { font-size: 8px; text-align: center; font-weight: bold; flex: 1; }
        
        .card-main-title { 
            text-align: center; margin: 2px 0; font-size: 18px; color: #1a5276; 
            font-family: 'Amiri', serif; letter-spacing: 2px;
        }

        .card-body { display: flex; padding: 5px 10px; }
        .user-photo { width: 65px; height: 80px; border: 2px solid #2575fc; background: #f8f9fa; display: flex; align-items: center; justify-content: center; font-size: 40px; color: #ccc; border-radius: 5px; }
        
        .user-info { flex: 1; padding-right: 10px; }
        .info-row { font-size: 11px; margin-bottom: 3px; display: flex; gap: 5px; }
        .info-row span { color: #555; width: 65px; }
        
        .card-side-meta { position: absolute; left: 10px; top: 65px; font-size: 7px; color: #777; text-align: left; }
        
        .card-footer-nin { 
            position: absolute; bottom: 0; width: 100%; background: #28a745; color: white; 
            text-align: center; font-size: 12px; font-weight: bold; padding: 3px 0; 
        }

        .card-qr { position: absolute; bottom: 30px; left: 10px; }
        .card-qr img { width: 35px; height: 35px; }

        /* ظهر البطاقة */
        .back { background: #fff; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 15px; text-align: center; }
        .back-title { color: #000; font-size: 16px; margin-bottom: 10px; border-bottom: 2px solid #28a745; width: 80%; }
        .back-rules { font-size: 9px; text-align: right; padding-right: 15px; line-height: 1.5; color: #333; }
        .back-seal { margin-top: 10px; font-weight: bold; font-size: 10px; color: #c0392b; border: 2px dashed #c0392b; padding: 5px; border-radius: 50%; opacity: 0.6; }
        .watermark-teacher { position: absolute; opacity: 0.05; width: 60%; top: 20%; z-index: 0; }
    </style>`;
}
...
function getCompactStyle() {
    return `
        <style>
            ...
            /* --- تنسيق الصورة الخلفية (Watermark) --- */
            .card-bg-watermark {
                position: absolute;
                top: 55%;
                left: 50%;
                ...
                width: 75%;
                opacity: 0.12; /* درجة الشفافية لضمان وضوح النص فوقها */
                z-index: 0;
                pointer-events: none;
            }

            /* تصميم الواجهة */
            .card-header { position: relative; z-index: 2; display: flex; align-items: center; padding: 5px; font-size: 8px; text-align: center; border-bottom: 1px solid #eee; background: rgba(255,255,255,0.7); }
            .card-header .icon { height: 20px; margin-left: 5px; }
            .card-title { position: relative; z-index: 2; text-align: center; font-weight: bold; font-size: 14px; background: rgba(240, 240, 240, 0.8); margin: 2px 0; }
            .card-content { position: relative; z-index: 2; display: flex; padding: 5px; gap: 10px; }
            .photo-placeholder { width: 60px; height: 70px; border: 1px dashed #ccc; display: flex; align-items: center; justify-content: center; font-size: 30px; color: #eee; background: rgba(255,255,255,0.5); }
            .details { font-size: 11px; line-height: 1.4; flex: 1; font-weight: bold; }
            .job-text { color: #d32f2f; font-weight: bold; }
            .card-footer { position: absolute; bottom: 0; width: 100%; background: #2e7d32; color: #fff; text-align: center; font-size: 10px; padding: 2px 0; z-index: 2; }
...
function printCardsFront() {
    ...
    let html = `${getCompactStyle()}<div class="print-grid">`;
    
    confirmedOnly.forEach(emp => {
        html += `
            <div class="card-front">
                <img src="https://i.ibb.co/p6S9X70/logo-back.png" class="card-bg-watermark">
                ...
                <div class="card-content">
                    ...
                    <div class="details">
                        <div><span>اللقب:</span> <b>${emp.fmn}</b></div>
                        <div><span>الاسم:</span> <b>${emp.frn}</b></div>
                        <div><span>الميلاد:</span> <b>${fmtDate(emp.diz)}</b></div>
                        <div class="job-text"><span>الوظيفة:</span> <b>${getJob(emp.gr)}</b></div>
                    </div>
                </div>
                <div class="card-footer">رقم التعريف الوطني: ${emp.nin || '------------------'}</div>
            </div>`;
    });
    html += `</div>`;
